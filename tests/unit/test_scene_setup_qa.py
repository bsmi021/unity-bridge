...
class TestSceneExtendedSchema:
    def test_schema_has_all_operations(self) -> None:
        from unity_bridge.mcp.schemas_phase3 import unity_scene_extended

        schema = unity_scene_extended()
    ...
    def test_schema_operations_match_csharp_handler(self) -> None:
        """Schema operation names must match what the C# handler accepts."""
        from unity_bridge.mcp.schemas_phase3 import unity_scene_extended

        schema = unity_scene_extended()
        ops = set(schema["properties"]["operation"]["enum"])
        # These are the operations the C# handler switch statement accepts
        expected_csharp = {
            "save",
            "restore",
            "list",
    ...
    def test_schema_has_setup_name(self) -> None:
        from unity_bridge.mcp.schemas_phase3 import unity_scene_extended

        schema = unity_scene_extended()
        assert "setupName" in schema["properties"]

    def test_schema_has_scene_path(self) -> None:
        from unity_bridge.mcp.schemas_phase3 import unity_scene_extended

        schema = unity_scene_extended()
        assert "scenePath" in schema["properties"]

    def test_schema_has_clear(self) -> None:
        from unity_bridge.mcp.schemas_phase3 import unity_scene_extended

        schema = unity_scene_extended()
        assert "clear" in schema["properties"]

    def test_schema_has_handle(self) -> None:
        from unity_bridge.mcp.schemas_phase3 import unity_scene_extended

        schema = unity_scene_extended()
        assert "handle" in schema["properties"]
        assert schema["properties"]["handle"]["type"] == "integer"
