...
def test_skill_description_stays_codex_discoverable() -> None:
    ...
    assert "MCP compatibility queueing" in description


def test_skill_mentions_cli_first_mcp_deprecation_and_queue() -> None:
    skill = _read(SKILL_PATH)

    assert "Use the CLI first" in skill
    assert "`unity-bridge serve` is deprecated MCP compatibility mode" in skill
    assert "unity_submit_command" in skill
    assert "unity_operation_status" in skill
