...
class TestMcpSchema:
    def test_schema_has_timeout(self) -> None:
        from unity_bridge.mcp.schemas_ext import player_settings

        schema = player_settings()
        assert "timeout" in schema["properties"]
        assert schema["properties"]["timeout"]["default"] == 15
    ...
    def test_schema_operation_enum_complete(self) -> None:
        ...
        assert enum_values == VALID_ACTIONS
    ...
    def test_schema_has_no_hardcoded_platform_enum(self) -> None:
        """M15: platform should NOT be a hardcoded enum in the schema."""
        from unity_bridge.mcp.schemas_ext import player_settings

        schema = player_settings()
        platform_prop = schema["properties"]["platform"]
        assert "enum" not in platform_prop

    def test_tool_registered_in_command_map(self) -> None:
        from unity_bridge.mcp.tools import TOOL_COMMAND_MAP

        assert "unity_player_settings" in TOOL_COMMAND_MAP
        assert TOOL_COMMAND_MAP["unity_player_settings"] == "player-settings-operation"

    def test_tool_registered_in_definitions(self) -> None:
        from unity_bridge.mcp.tools import TOOL_DEFINITIONS

        names = [t["name"] for t in TOOL_DEFINITIONS]
        assert "unity_player_settings" in names
