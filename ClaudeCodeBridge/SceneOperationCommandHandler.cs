...
namespace BWS.Editor.ClaudeCodeBridge
{
    ...
    /// </summary>
    public partial class SceneOperationCommandHandler : ICommandHandler
    {
        ...
        private SceneOperationResult LoadScene(SceneOperationParams parameters)
        {
            ...
            {
                if (!BridgeSceneModalRecovery.PrepareForExplicitSave("scene-operation load save-current",
                    out var saveModalMessage))
                {
                    result.success = false;
                    result.message = saveModalMessage;
                    return result;
                }
            ...
            {
                var ready = BridgeSceneModalRecovery.PrepareForAutomation("scene-operation load",
                    out var modalMessage);
                if (!ready)
                {
                    result.success = false;
                    result.message = modalMessage;
                    return result;
                }
            }
        ...
        private SceneOperationResult CreateScene(SceneOperationParams parameters)
        {
            ...
            if (parameters.saveCurrentScene)
            {
                if (!BridgeSceneModalRecovery.PrepareForExplicitSave("scene-operation create save-current",
                    out var saveModalMessage))
                {
                    result.success = false;
                    result.message = saveModalMessage;
            ...
            var ready = BridgeSceneModalRecovery.PrepareForAutomation("scene-operation create",
                out var modalMessage);
            if (!ready)
            {
                result.success = false;
