...
- Typed CLI paths now expose the existing animation `set-curve`, `add-event`, and `set-properties`; terrain `set-heights` and `set-settings`; tilemap `fill-box` and `compress-bounds`; and `asset-ext reserialize` operations.
- Added a standalone Mono.Cecil Unity API inventory/diff/coverage-registry tool, a generated C#/Python/CLI command-parity gate, and environment-selected Unity 6000.5 live fixture roles.
- The privileged `script` escape hatch now supports exact assembly requests, structured result schemas, separated compiler diagnostics and Unity logs, declared execution intent, explicit internal-reflection opt-in, and repeatable declared `GlobalObjectId`/`Assets/` mutation targets with pre-execution Undo/file backups and verified rollback reporting.
- Added `script-job`/`script-cancel` for cooperative one-step-per-update C# jobs with deadlines, cancellation, durable terminal delivery, and truthful reload interruption.
- Added `script-probe-assemblies` plus exact assembly-scoped generic proof ingestion in `UnityApiInventory registry-build`; the pinned Unity 6.5 core-plus-Builder snapshot now proves all 199 public-surface assemblies and classifies all 129,615 records.
- Added `unity-bridge profile platforms` to discover the installed Unity 6.5 Build Profile platform GUIDs required by `profile create`.
...
### Changed
- The Unity 6.5 coverage implementation is packaged with one explicit limitation: the positive Addressables 3.1.0 content-build fixture is not counted as covered because the validation machine's Unity 6000.5.1f1 UPM fails before package registration. The real negative build path and all other required live scenarios remain proven.
- `import-settings bulk-set` now wraps bulk importer updates in `AssetDatabase.StartAssetEditing()` / `StopAssetEditing()` with a `finally` guard.
- `HeartbeatGenerator` now adds Unity 6.5 lifecycle attribute callbacks under `UNITY_6000_5_OR_NEWER` while keeping legacy editor event hooks as fallback.
- The API inventory now uses exact `UnityEditor.*Module.dll` runtime modules instead of the monolithic reference facade, so snapshot MVIDs and hashes can be selected in the live Editor.
...
- The `unity-bridge-cli` skill (`.agents/skills/unity-bridge-cli`, the canonical copy) now documents when to escalate to the unrelated third-party `unity-mcp` MCP server for capabilities this bridge doesn't have (script CRUD/AST edits, inline/multi-angle screenshots, per-sample profiler drill-down, AI asset generation, external model import, SHA preconditions), and defaults to the CLI for everything else.
...
### Fixed
- Package metadata no longer requests Typer's removed `all` extra, eliminating the warning emitted by current Typer releases during clean wheel installation.
- Command parity now preserves editor focus/refresh flags, component field filters and updates, console regex/stack limits, profiler CPU sampling, positioned prefab instantiation, and prefab-instance destruction across Python/C# contracts. Existing build platform list/switch handlers are now dispatched and exposed as exact `build` subcommands.
- Console regex filters use a bounded match timeout, compile requests forward their Unity-side timeout and persist reload recovery before refresh, and inner-payload failures reconcile an already-completed cross-process ledger record to failed.
- Asset model import, hashing, and script edits now share canonical `Assets/` path containment that rejects traversal and symlink/junction escapes. Model imports refuse overwrite by default, expose explicit `--overwrite`, and roll back asset/`.meta` state; failed script imports likewise restore the prior script and metadata.
- Successful bridge envelopes containing `data.success=false` now become failed CLI results and failed operation-ledger states instead of exiting zero.
- Multi-angle screenshots now apply each Scene View angle instantaneously and restore pivot, rotation, size, projection, and 2D mode in `finally`.
- Unity 6.5 Build Profile creation now remains `running` until its callback writes the terminal result. Profiler capture now drives the Editor `ProfilerDriver`, auto-stops at the requested frame budget with retained-frame/overshoot telemetry, and cancels an armed frame callback when stopped through `profiler-control`.
- Build Profile creation now uses a Unity `ScriptableObject` callback target and a bounded verified-persistence fallback, fixing invalid callback registration while preserving truthful terminal completion.
- Installer reconciliation ignores Unity-generated `.meta` sidecars for managed non-asset files such as `AGENTS.md`, while still pruning genuinely obsolete managed files.
- Detached submission now fails if its initial operation record cannot be persisted; response publication records the C# ledger terminal state first; and empty serialized terminal timestamps are normalized and repaired.
- CLI output normalization now preserves dimensional names such as `is2D` as `is_2d` without corrupting identifiers such as `sha256`.
- `menu --validate-only` now sends the C# handler's `validate` field, and screenshot `--camera` now sends `cameraPath`, so neither option is silently ignored by Unity.
- Protocol timeout and parallel-safety policy now names the registered `bridge-status` command instead of the retired `health-check` identifier.
...
- Heartbeat and operation-ledger readers now tolerate longer Windows atomic-replace lock windows instead of randomly rejecting high-frequency command/proof runs.
- Operation-ledger reads now also retry the transient missing-file window during a cross-process atomic replacement instead of reporting an in-flight detached operation as permanently not found.
- PlayMode control now persists enter/stop transitions through domain reload and writes the real terminal response. Heartbeat readiness tracks only lifecycle transitions rather than treating the entire Play session as command-busy, so `playmode stop` remains dispatchable while playing.
...
- `unity-bridge install` now prunes obsolete managed bridge files from `Assets/Scripts/Editor/ClaudeCodeBridge/`, so removed or renamed C# handlers do not remain in installed Unity projects after an update.
- `unity-bridge clean --all` now preserves command/response files referenced by active operation records instead of deleting live bridge state while keeping the active ledger snapshot.
...
- glTF/GLB import validation now recognizes Unity 6.5's base `AssetImporter` as the unsupported fallback, returning a truthful failure and rolling back both the staged asset and `.meta` instead of accepting a `DefaultAsset` as an imported model.
...
- **C# bridge: `GetSelectionCommandHandler` and `ValidatePrefabCommandHandler` now compile under Unity 6.5.** `Object.GetInstanceID()` and `SerializedProperty.objectReferenceInstanceIDValue` are obsolete-as-error in 6.5, replaced by `Object.GetEntityId()` and `SerializedProperty.objectReferenceEntityIdValue` (Unity's InstanceID → EntityId migration). Both handlers now branch on `#if UNITY_6000_5_OR_NEWER` (matching the existing `UNITY_6000_0_OR_NEWER` gating convention elsewhere in the bridge) to keep compiling on both pre-6.5 and 6.5+ editors. `get-selection`'s `instanceId` field changes from a JSON number to a numeric string on every Unity version (not just 6.5+), since `EntityId` is a 64-bit value that would silently truncate if narrowed to `int` -- this mirrors the string convention `object-identity`'s `entityId` field already uses.
...
### Added
- Unity 6.5 migration surfaces: `script-edit` range/anchor MonoScript edits with SHA256 preconditions, `asset-ext hash`, `asset-ext import-model`, `profile create`, `profiler-frame`, `profiler-control set-areas`, screenshot `--inline-base64`, and screenshot `--multi-angle`.
- `unity-bridge install --include-claude`: additionally links `.claude/skills/unity-bridge-cli` to the canonical `.agents/skills/unity-bridge-cli` directory (a real symlink, falling back to an NTFS junction on Windows when symlink privilege is unavailable), for Claude Code -- which, unlike Codex and GitHub Copilot, does not scan `.agents/skills` natively. Off by default; Codex and Copilot need no extra step. New `core/skill_links.py` module handles cross-platform link creation/detection and refuses to touch a target path that isn't already a link it manages.
- `unity-bridge install --check` now also reports Claude Code skill-link status (`skill.claude_link.linked`).
